...
# ###(yolov5) D:\bhanu\OneDrive - Imagevision.ai India Pvt Ltd\bhanu_iv061\Packaging\Olamagri\engineering\code_v1\yolov5-master>python seg_y_axis_counter.py  --imgsz 640 --weights "D:\bhanu\OneDrive - Imagevision.ai India Pvt Ltd\bhanu_iv061\Packaging\Olamagri\engineering\code_v1\models_video_1\olam_agri_video_2_seg.pt" --conf-thre 0.60 --project "D:\bhanu\olam_agri\ai_infernces" --name olam_agri_v1_ --source "C:\Users\admin\Downloads\olam_agri_vid_1.mp4" --draw-line
...
def run(
    ...
    try:
        ...
        for data in tqdm(dataset, desc="Segmentation Counting"):
            ...
            if not line_selected:
                global LINE_POINTS
                print("Manual setup: click 4 points in order -> C1:P1, C1:P2, C2:P3, C2:P4")
                LINE_POINTS = []
                cv2.namedWindow(window_name)
                cv2.setMouseCallback(window_name, _line_mouse_callback)
                ...
                while True:
                    ...
                    for i, pt in enumerate(LINE_POINTS):
                        dot_color = (0, 255, 255) if i < 2 else (255, 255, 0)
                        cv2.circle(preview, pt, 5, dot_color, -1)

                    if len(LINE_POINTS) >= 2:
                        cv2.line(preview, LINE_POINTS[0], LINE_POINTS[1], (0, 255, 255), 2)
                        cv2.putText(preview, "Conveyor 1", (LINE_POINTS[0][0], max(20, LINE_POINTS[0][1] - 10)),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                    if len(LINE_POINTS) >= 4:
                        cv2.line(preview, LINE_POINTS[2], LINE_POINTS[3], (255, 255, 0), 2)
                        cv2.putText(preview, "Conveyor 2", (LINE_POINTS[2][0], max(20, LINE_POINTS[2][1] - 10)),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)

                    cv2.putText(preview, "L-Click: C1 P1,P2 then C2 P3,P4 | R-Click: reset", (15, 30),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                    cv2.putText(preview, f"Points selected: {len(LINE_POINTS)}/4", (15, 60),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                    cv2.imshow(window_name, preview)
                    key = cv2.waitKey(20) & 0xFF
                ...
                if len(LINE_POINTS) == 4:
                    ...
                    print(f"Selected Conveyor 1: P1=({p1x}, {p1y}), P2=({p2x}, {p2y})")
                    print(f"Selected Conveyor 2: P3=({p3x}, {p3y}), P4=({p4x}, {p4y})")
                line_selected = True
